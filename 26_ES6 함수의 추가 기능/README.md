...
화살표 함수는 함수 호이스팅이 되지 않는 이유:
  ...
  1. 함수 선언문으로 정의한 함수와 함수 표현식으로 정의한 함수는 함수 생성 시점이 다르다. 함수 선언문으로 함수를 정의하면 런타임 이전에 함수 객체가 이미 생성이 되는 반면, 함수 표현식으로 함수를 정의하면 런타임(즉 해당 할당문 실행 시점)에 함수 객체가 생성된다. (참고: [함수 호이스팅](https://poiemaweb.com/js-function#2-%ED%95%A8%EC%88%98-%ED%98%B8%EC%9D%B4%EC%8A%A4%ED%8C%85))
