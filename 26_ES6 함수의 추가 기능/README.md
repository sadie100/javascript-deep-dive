...
- 기존 함수보다 **정의, 내부 동작이 간략함**
- 화살표 함수는 콜백 함수 내부에서 **`this`가 전역 객체를 가리키는 문제를 해결하기 위한 대안**으로 유용함
## 26.3.1 화살표 함수 정의
### 함수 정의
- 함수 표현식으로 정의
- 호출 방식은 기존 함수와 동일
...
- 이때 함수 몸체 내부의 문이 **값으로 평가될 수 있는 표현식**이면 <mark style="background: #FFB8EBA6;">`return`을 생략해도 알아서 반환 됨</mark>
- 하지만! 하나의 문으로 구성되어도 **표현식이 아닌 문**은 에러 → 반환 값이 없기 때문 → **따라서 중괄호 `{}`를 생략할 수 없음**
```js
/* 함수 몸체가 하나의 문으로 구성된 화살표 함수 예시 */ 
...
// 위 표현은 아래와 동일
const arrow = () => { return const x = 1; }; // error; 표현식이 아니므로 반환 값이 없다
// 다음과 같이 return 없이 표현하는 것이 적절함
